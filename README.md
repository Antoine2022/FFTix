...
demos : folder with demos
 ...
 - non_linear_polycrystal.py : example of a computation on a polycrystal, using a MFront material behaviour on each grain. The file "crystalline.mfront" must be compiled with mfront by doing "mfront --obuild --interface=generic crystalline.mfront" and then, to launch the demo (but also to use Non_linear_schemes.py), the mgis module must be sourced. Hence, you must install MFrontGenericInterfaceSupport.
...
And another example on on polycrystal whose strain-rate is governed by one non-linear potential (demo non_linear_polycrystal.py):
